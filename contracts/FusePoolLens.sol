  ...
  function initialize(
    ...
    require(
      ...
      "No uniswap LP token names supplied or array lengths not equal."
        ...
        for (uint256 i = 0; i < _hardcodedAddresses.length; i++) {
            hardcoded[_hardcodedAddresses[i]] = TokenData({
                name: _hardcodedNames[i],
                symbol: _hardcodedSymbols[i]
            });
        }
        ...
        returns (FusePoolData[] memory, bool[] memory)
    {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            try
                this.getPoolSummary(IComptroller(pools[i].comptroller))
            returns (
        ...
        )
    {
        ...
        for (uint256 i = 0; i < cTokens.length; i++) {
            ...
            totalBorrow =
                totalBorrow +
                (assetTotalBorrow * underlyingPrice) /
                1e18;
            totalSupply =
                totalSupply +
                (assetTotalSupply * underlyingPrice) /
                1e18;
        ...
        returns (string memory, string memory)
    {
        ...
        for (uint256 i = 0; i < uniswapData.length; i++) {
            try IUniswapV2Pair(token).token0() returns (address _token0) {
                ...
                bool isUniswapToken = keccak256(abi.encodePacked(_name)) ==
                    keccak256(abi.encodePacked(ud.name)) &&
                    keccak256(abi.encodePacked(_symbol)) ==
                    keccak256(abi.encodePacked(ud.symbol));
     ...
     * @return An array of Fuse pool assets.
     */
    function getPoolAssetsWithData(IComptroller comptroller)
        external
        ...
        )
    {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            for (uint256 j = 0; j < assets.length; j++) {
                totalBorrow =
                    totalBorrow +
                    (assets[j].borrowBalance * assets[j].underlyingPrice) /
                    1e18;
                if (assets[j].membership) {
                    totalCollateral =
                        totalCollateral +
                        (((assets[j].supplyBalance *
                            assets[j].underlyingPrice) / 1e18) *
            ...
            uint256 health = totalBorrow > 0
                ? (totalCollateral * 1e18) / totalBorrow
                : 1e36;
        ...
        )
    {
        ...
        // Scope to avoid "stack too deep" error
        {
            ...
            for (uint256 i = 0; i < publicPools.length; i++)
                comptrollers[i] = IComptroller(publicPools[i].comptroller);
        }
     ...
     * @dev This function is not designed to be called in a transaction: it is too gas-intensive.
     */
    function getPoolAssetsByUser(IComptroller comptroller, address user)
        public
        returns (FusePoolAsset[] memory)
    {
        FusePoolAsset[] memory assets = getPoolAssetsWithData(
            comptroller,
            comptroller.getAssetsIn(user),
            user
        );
        return assets;
    }
    ...
    function verifyPrice(
        ...
        return
            IKeydonixUniswapTwapPriceOracle(address(oracle)).verifyPrice(
                cToken,
                proofData
            );
    }
